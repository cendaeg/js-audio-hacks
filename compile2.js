...
function startOne() {
  ...
  var numChannels = 2,
      lengthInSamples = parseInt(lengthField.value, 10),
      sampleRate = 44100;
  ...
  var node = new Waver(offlineCtx, 32, function (t) {
    var i = new Instrument(t);
    var note = random_score[Math.floor(t % random_score.length)];
    return (0 + i.chord(note.freq) + i.bass(note.freq * (3 / 2) / 2) + i.lowDrums(100)) / 3;
  });
